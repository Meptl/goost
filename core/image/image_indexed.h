...
public:
	...
	Error create_indexed_from_data(const Vector<uint8_t> &p_palette_data, const Vector<uint8_t> &p_index_data);
	...
	void set_palette(const Vector<Color> &p_palette);
	Vector<Color> get_palette() const;

	void set_palette_color(int p_idx, const Color p_color);
	Color get_palette_color(int p_idx) const;

	Vector<uint8_t> get_palette_data() const;
	Vector<uint8_t> get_index_data() const;
